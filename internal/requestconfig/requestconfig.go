...
import (
	...
	"scalar-api/internal/apiform"
	"scalar-api/internal/apiquery"
...
func getDefaultHeaders() map[string]string {
	return map[string]string{
		"User-Agent": "ScalarAPI/Go " + internal.PackageVersion,
	}
}
...
type RequestConfig struct {
	...
	Body         io.Reader
	// WebhookSecret holds the secret used to verify inbound webhook signatures.
	// It is a pre-request option (set via option.WithWebhookSecret or the
	// client environment) and is never sent on outbound requests.
...
// maxDiscardBytes bounds how much of an unread response body is drained before
// closing it. Draining lets the HTTP client reuse the connection; the bound
// keeps a misbehaving server from streaming unbounded data into the discard.
const maxDiscardBytes = 512 << 10
...
func (cfg *RequestConfig) Execute() (err error) {
	...
	if res == nil {
		return fmt.Errorf("requestconfig: http client returned nil response")
	}
	if res.Body == nil {
	...
	if cfg.ResponseBodyInto == nil && cfg.ResponseInto == nil {
		// Nothing will ever read this body (e.g. a DELETE/204 method with no
		// destination): drain a bounded amount so the connection can be reused,
		// then close it instead of leaking it. The deferred cancel still fires.
		_, _ = io.Copy(io.Discard, io.LimitReader(res.Body, maxDiscardBytes))
		_ = res.Body.Close()
		return nil
	}
	if cfg.ResponseBodyInto == nil || intoCustomResponseBody {
		// The body is handed to the caller (WithResponseInto or a **http.Response
		// destination), so we aren't reading it in this scope — but whoever is will
		// need the cancel func from the context to observe request timeouts.
...
func (cfg *RequestConfig) Clone(ctx context.Context) (*RequestConfig, error) {
	if cfg == nil {
		return nil, fmt.Errorf("requestconfig: cannot clone nil config")
	}
	req := cfg.Request.Clone(ctx)
	var err error
	if req.Body != nil {
	...
	// Body, ResponseBodyInto, and ResponseInto are intentionally not copied:
	// pagination's GetNextPage re-establishes the destinations for the next
	// request, and the request body is read back from the cloned [*http.Request]
	// (see [RequestConfig.WriteJSONBodyField]).
	new := &RequestConfig{
...
// WriteJSONBodyField sets key to a raw, already-serialized JSON token in the request's JSON body,
// preserving any existing fields, and prepares the request so the mutated body reaches the wire.
//
// It exists so callers (e.g. cursor pagination placing a cursor on the request body) do not have to
// reach into the cached [*http.Request]: this method owns the body bookkeeping that [RequestConfig.Clone]
// and [RequestConfig.Execute] imply. Clone does not copy [RequestConfig.Body], so the current body is
// read back from the cloned request, the field is merged in via sjson (a raw token keeps its exact
// JSON form — a large numeric id is not rounded), the body is labelled application/json, and the
// cached request body is cleared so Execute rebuilds it from [RequestConfig.Body].
func (cfg *RequestConfig) WriteJSONBodyField(key string, rawJSON string) error {
	existing, err := cfg.currentBodyBytes()
	if err != nil {
		return err
	}
	merged, err := sjson.SetRawBytes(existing, key, []byte(rawJSON))
	if err != nil {
		return err
	}
	cfg.Body = bytes.NewBuffer(merged)
	cfg.Request.Header.Set("Content-Type", "application/json")
	// Execute rebuilds Request.Body/ContentLength/GetBody from cfg.Body only when Request.Body is
	// nil; Clone restored Request.Body from GetBody, so clear it here for the rebuild to run.
	cfg.Request.Body = nil
	cfg.Request.ContentLength = 0
	cfg.Request.GetBody = nil
	return nil
}

// currentBodyBytes returns the request's current body bytes: [RequestConfig.Body] when it is a
// buffer, otherwise the cached request's replayable body via GetBody. It returns nil (and no error)
// when the request carries no body yet, so a field can still be written onto an empty body.
func (cfg *RequestConfig) currentBodyBytes() ([]byte, error) {
	if buffer, ok := cfg.Body.(*bytes.Buffer); ok {
		return buffer.Bytes(), nil
	}
	if cfg.Request != nil && cfg.Request.GetBody != nil {
		rc, err := cfg.Request.GetBody()
		if err != nil {
			return nil, err
		}
		if rc == nil {
